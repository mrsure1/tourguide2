...
export async function GET(request: Request) {
  ...
  if (popup && orderId) {
    const redirectUrl = new URL(`/payment-popup/${orderId}`, origin);
  ...
  if (orderId) {
    const redirectUrl = new URL(`/traveler/bookings/checkout/${orderId}`, origin);
    redirectUrl.searchParams.set("error", code);
    redirectUrl.searchParams.set("message", message);
    return NextResponse.redirect(redirectUrl);
  }

  const fallbackUrl = new URL("/payment-popup", origin);
